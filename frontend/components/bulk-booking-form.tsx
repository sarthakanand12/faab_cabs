...
import { MapPin, Calendar, Clock, Phone, Car, ChevronDown, ArrowRight, Minus, Plus } from "lucide-react"
...
export function BulkBookingForm({ onSubmit }: BulkBookingFormProps) {
    ...
    const decrementVehicles = () => setVehicleCount(prev => Math.max(1, prev - 1))
        ...
        <form onSubmit={handleSubmit}>
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center justify-between h-12 bg-secondary/50 border border-border rounded-md px-3">
                        <span className="text-sm text-muted-foreground font-medium">{t("total_vehicles")}</span>
                        <div className="flex items-center gap-3">
                            <button
                                type="button"
                                onClick={decrementVehicles}
                                className="h-8 w-8 flex items-center justify-center rounded-full bg-background border border-border hover:bg-secondary transition-colors"
                                disabled={vehicleCount <= 1}
                            >
                                <Minus className="h-4 w-4" />
                            </button>
                            <span className="font-bold min-w-[20px] text-center">{vehicleCount}</span>
                            <button
                                type="button"
